...
// Push constants for DeferredLighting.frag (grid + ambient; specular deferred until world-pos G-buffer).
struct Gfx_DeferredLightingPushConstants {
    ...
    float    viewWorldPos[ 4 ];  // reserved for future specular
};
...
static_assert( sizeof( Gfx_DeferredLightingPushConstants ) == 48, "push constants must match DeferredLighting.frag" );
