...
public:
    ...
    const Util_InputSnapshot& GetSnapshot() const {
        return mySnapshot;
    }

    // High-resolution time when Sample() finished (for input→present latency).
    std::chrono::high_resolution_clock::time_point GetLastSampleTime() const {
        return myLastSampleTime;
    }
    bool HasLastSampleTime() const {
        return myHasLastSampleTime;
    }
