...
**Migration note (ownership WIP):** HybridDeferred **pass recording** for AO / Hi-Z / Cluster / Soft / SSR / DDGI / Deferred draw / Post / ShadowMap lives in `Gfx_*Pass` via `Rhi_CommandList`. **GBuffer/hybrid Begin/End** also live in `Gfx_HybridDeferredPass` (O1). Rhi now includes a **compute create surface** (sampler, descriptor set layout/pool/allocate, pipeline layout, compute pipeline, mip views, descriptor image updates); DepthPyramid **Init** creates PSO/descriptors through Rhi (pyramid image still RC VMA). Thin `Vk_*_Record` facades and most pass Init still in RenderCore. **Epic:** [`gfx-rhi-ownership-completion_Plan.md`](gfx-rhi-ownership-completion_Plan.md).
