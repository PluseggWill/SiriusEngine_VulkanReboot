...
**Migration note (locked 2026-07-24):** HybridDeferred **Record + Init** for AO / Hi-Z / Cluster / Soft / SSR / DDGI / Deferred draw / Post (images+compute+tonemap) / ShadowMap live in `Gfx_*Pass` via Rhi. **GBuffer/hybrid Begin/End** live in `Gfx_HybridDeferredPass`. FG Records bind `myGfx` handles; DescriptorSystem samples shadow via `myShadowMapState.myGfx`. Remaining RenderCore `Vk_*Pass` TUs are orchestration only: SPIR-V file load, WSI hybrid RP/FB + swapchain tonemap FB adopt, cross-pass GBuffer/IBL/shadow adopt for descriptor updates, CPU layout/history trackers (TAA/SSR/AO/DDGI/shadow depth). GBuffer/IBL image ownership and Forward path remain RenderCore. Closed epic: [`Archived/plans/gfx-rhi-ownership-completion_Plan.md`](Archived/plans/gfx-rhi-ownership-completion_Plan.md).
